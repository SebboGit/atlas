...
every transition by a day. An unknown `CRON_TZ` falls back to UTC with a
`worker.schedules.invalid_tz` warning rather than failing the boot. Cron
expressions use the standard 5-field form.
