...
- Anything that happens during a trip. Discriminated union:
  ...
  - `transit` — non-flight transit by `mode` (`train` | `bus` | `ferry` | `car` | `other`): carrier, `fromName` / `toName`, reference number. `address` and `plusCode` locate the destination. `train`, `bus`, and `ferry` treat the leg as two endpoints and locate the origin with `fromAddress` / `fromPlusCode`; `car` and `other` keep a single pin and ignore those fields. A legacy row with only a `fromName` plus an `address` or `plusCode` reads them as the origin's. Transit keeps a single `countryCode`. See ADR-0019.
