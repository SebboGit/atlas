...
- **Station keys:** a train, bus or ferry endpoint with only a name is keyed `station:<mode>:<cc>:<name>`, which `PlaceResolver` routes ahead of Plus Codes and free text. It tries tag-filtered Photon behind a name-agreement guard, then the raw name through Nominatim and Photon. The country is the segment's ISO code, never `locationName`; car and other modes keep plain name queries (ADR-0019).
...
- **ADR-0019** — Station-aware transit endpoints: train/bus/ferry legs get `fromAddress`/`fromPlusCode` for the origin, `station:` cache keys resolved by tag-filtered Photon behind a name guard with a raw-name fallback, a station-to-station map line, and a plain Google Maps Directions link (no API). Car/other keep one pin and transit keeps one country; Transitous and Google routing APIs rejected. Accepted.
