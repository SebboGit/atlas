...
- **Station keys:** a train, bus or ferry endpoint with only a name is keyed `station:<mode>:<cc>:<name>`, which `PlaceResolver` routes ahead of Plus Codes and free text. It tries tag-filtered Photon behind a name-agreement guard, then the raw name through Nominatim and Photon. The country is the segment's ISO code, never `locationName`; car and other modes keep plain name queries (ADR-0019). An end that only the free-text fallback could place and that sits further from the other end than its mode's fallback threshold keeps its pin but loses the route line, and the leg says which end looks wrong.
...
- **Derived codes are display-only:** a Plus Code encoded from cached coordinates is never written into a form field. The edit dialog shows it as a line under the Plus Code field while the values would still geocode to the same cache row, because a stored code outranks the name and would freeze the pin against every later edit (ADR-0019's clause, generalised to hotel, food and activity). A code the user types or picks is stored and keeps precedence.
...
- **ADR-0018** — Photon-first free-text geocoding with Nominatim fallback; hotels/food flip to venue-name-first queries with a locationName-else-country context tail. Google Places rejected on ToS grounds (non-Google-map display, coordinate-caching limits). Accepted (transit station clause amended by ADR-0019).
- **ADR-0019** — Station-aware transit endpoints: train/bus/ferry legs get `fromAddress`/`fromPlusCode` for the origin, `station:` cache keys resolved by tag-filtered Photon behind a name guard with a raw-name fallback, a station-to-station map line, and a plain Google Maps Directions link (no API). Car/other keep one pin and transit keeps one country; Transitous and Google routing APIs rejected. Accepted (source-aware fallback distance guard on the route line added, 2026-09-18; display-only derived-code clause generalised to every pinned type, 2026-09-18).
