...
- **ADR-0019** — Station-aware transit endpoints: train/bus/ferry legs get `fromAddress`/`fromPlusCode` for the origin, `station:` cache keys resolved by tag-filtered Photon behind a name guard with a raw-name fallback, a station-to-station map line, and a plain Google Maps Directions link (no API). Car/other keep one pin and transit keeps one country; Transitous and Google routing APIs rejected. Accepted (source-aware fallback distance guard on the route line added, 2026-09-18).
